...
char *cmdBMP_Start = "BMPON"; //蜂鸣器打开
char *cmdBMP_Stop  = "BMPOFF"; //蜂鸣器关闭
	
char *ledFlag = "false";       //LED状态
	...
	{	
		...
		{     
			...
			{                
				...
				{    
					int status = Read_MQ4_DOUT_Data(); //获取是否检测到可燃气体的状态
					if(!status){//status == 0 表示检测到可燃气体
						Send_beepData();//发送检测数据给服务器
				}
					if(!ReadFlameSensorStatus()){//获取是否检测到火焰的状态,如果为0则是检测到有火焰
						Send_FireData();//发送检测到火焰的数据给服务器
						DelayMs(1000);
						u1_printf("进入if");
					}
			...
			{           				
				...
				if(!memcmp(&MQTT_CMDOutPtr[2],cmdLED_Off,strlen(cmdLED_Off)))//判断指令，如果是CMD1 
				{      
					u1_printf("收到关灯命令\r\n");
					ledFlag = "false";  								             
					LED_Off();									  		   //LED关闭
				...
				else if(!memcmp(&MQTT_CMDOutPtr[2],cmdLED_On,strlen(cmdLED_On))) //判断指令，如果是CMD1
				{ 
					u1_printf("收到开灯命令\r\n");
					ledFlag = "true";                                              
					LED_ON(); 	      //LED开启
        ...
        else if(!memcmp(&MQTT_CMDOutPtr[2],cmdBMP_Start,strlen(cmdBMP_Start)))	
				{
				     bmp_start();
				}			
				 else if(!memcmp(&MQTT_CMDOutPtr[2],cmdBMP_Stop,strlen(cmdBMP_Stop)))	
				{
				     bmp_stop();
				}	
				//不做处理，后面会发送状态
